...
Open `index.html`. That is the whole instruction — no build step, no server, no dependencies, no storage. **≈1.38 MB, one file, and it runs from `file://`.**

**It reads in English, Spanish and Brazilian Portuguese** (Reading & Access → Language). Every string a student sees is in
...
**The Spanish and Portuguese are drafts by Claude that no fluent speaker or teacher has read** — the picker says so,
