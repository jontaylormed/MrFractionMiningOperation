...
[![Checks](https://img.shields.io/badge/MF.validate()-15%2C928%20checks-4A6FA5?style=flat-square)](docs/VERIFICATION.md)
...
**Six layers**, by ore richness: plain numbers → shared metal → two-term ore and pure ingots → `x² + bx + c` → `ax² + bx + c` → **four terms in two letters, `xy + 3x + 2y + 6`**. Three further shafts are cut but not yet timbered, and are named on screen so you know the mine keeps going.
...
**15,928 checks across 44 groups, 0 errors, and two controls that must fail — and do.** It sweeps the factor engine, seam classification, **hundreds of generated lumps across the six layers** (the ore is generated, not authored, so nobody has ever seen most of it), the printer, the pour, the forge, the Casting Shed's three workshops, every tool's reading and lesson, colour contrast on every rendered surface, the reading-and-access panel, the sound layer, the seam's whole length, the doors on the surface, the no-gate rule, and the no-grades rule.
