...
[![Checks](https://img.shields.io/badge/MF.validate()-21%2C835%20checks-4A6FA5?style=flat-square)](docs/VERIFICATION.md)
...
**Eight layers**, by ore richness: plain numbers → shared metal → two-term ore and pure ingots → `x² + bx + c` → `ax² + bx + c` → **four terms in two letters, `xy + 3x + 2y + 6`** → **`x⁴ + bx² + c`, a trinomial one gear up** → **`x² + bxy + cy²`, the same hunt in two letters**. One further shaft is cut but not yet timbered, and are named on screen so you know the mine keeps going.
...
**21,835 checks across 46 groups, 0 errors, and two controls that must fail — and do.** It sweeps the factor engine, seam classification, **hundreds of generated lumps across the eight layers** (the ore is generated, not authored, so nobody has ever seen most of it), the printer, the pour, the forge, the Casting Shed's three workshops, every tool's reading and lesson, colour contrast on every rendered surface, the reading-and-access panel, the sound layer, the seam's whole length, the doors on the surface, the no-gate rule, and the no-grades rule.

**Every check carries a control that must fail**, and a new one is proved by reintroducing the fault it exists to catch — not by watching it pass. `docs/VERIFICATION.md` is 99 rules, each written after a real failure.
...
docs/VERIFICATION.md             99 rules, each written after a real failure
...
- `docs/VERIFICATION.md` is the collaboration's actual output: 99 rules, each one written
