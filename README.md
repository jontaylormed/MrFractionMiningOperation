...
[![Checks](https://img.shields.io/badge/MF.validate()-25%2C918%20checks-4A6FA5?style=flat-square)](docs/VERIFICATION.md)
...
**25,918 checks across 47 groups, 0 errors, and two controls that must fail — and do.** It sweeps the factor engine, seam classification, **hundreds of generated lumps across the nine layers** (the ore is generated, not authored, so nobody has ever seen most of it), the printer, the pour, the forge, the Casting Shed's three workshops, every tool's reading and lesson, colour contrast on every rendered surface, the reading-and-access panel, the sound layer, the seam's whole length, the doors on the surface, the three claims the front page makes to a stranger, the no-gate rule, and the no-grades rule.

**Every check carries a control that must fail**, and a new one is proved by reintroducing the fault it exists to catch — not by watching it pass. `docs/VERIFICATION.md` is 100 rules, each written after a real failure.
...
docs/VERIFICATION.md             100 rules, each written after a real failure
...
- `docs/VERIFICATION.md` is the collaboration's actual output: 100 rules, each one written
