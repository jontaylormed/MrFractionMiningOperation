...
**Last verified 2026-08-29** by `MF.validate()` in a browser over a **local HTTP server** at **380, 560, 994 and 1250px**, by `MF.playthrough(4)` and `(5)`, and by a hand-driven contrast sweep of **145 mine states at each of those four widths — 580 renders** (`VERIFICATION.md` §67). **`file://` could not be used:** the preview pane renders it as a static snapshot and `MF` never runs, so nothing on the page can be exercised at all.
...
| **Validation** | `MF.validate()` → **9,944 checks / 34 groups / 0 errors**, two controls that must fail, and do — **run it at the width you ship from**, because `layout`, `reach`, `hollow` and `cartdraw` measure the live viewport and report it (`VERIFICATION.md` §61, §63, §65). Verified at **380×780, 560×760, 994×700 and 1250×900** |
...
- **The mine has been swept for contrast and the sweep had edges.** 145 states × 4 widths, one failure found and fixed (`VERIFICATION.md` §67). It measured **text against its backdrop, at rest**: not `:hover` or `:focus` colours, not disabled controls, not the forge or the workshops, and nothing mid-transition beyond the two burst frames forced visible. **And contrast is not legibility** — size, weight, tracking and whether a sentence is worth reading are human questions, and none of them has been asked of one line on this site.
